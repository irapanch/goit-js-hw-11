...
async function onFormSubmit(event) {  //виконується при відправленні форми
  ...
  loadBtn.classList.add('is-hidden'); // спочатку приховуємо кнопку прокрутки
    try {
      ...
      totalPage = Math.ceil(response.data.totalHits / per_page); // розрахунок кількості сторінок
      renderGallery(response.data.hits); // відображення галереї зображень  
      if (response.data.total === 0) { // перевірка кількості знайдених зображень. Якщо бекенд повертає порожній масив, значить нічого підходящого не було знайдено 
        Notify.failure( 
      ...
      if (response.data.totalHits > 0) {  // перевірка кількості знайдених зображень. 
        Notify.info(`Hooray! We found ${response.data.totalHits} images.`, { // виведення повідомлення з кількістю знайдених зображень
          timeout: 600,
        });
        if (response.data.totalHits <= per_page) {  // перевірка кількості знайдених зображень. 
...
async function onClickLoadBtn() { // виконується при кліку на кнопку прокрутки
  ...
  try {
    const response = await getImages(page, query);  // виконується асинхронний запит до getImages з передачею значення page
    renderGallery(response.data.hits);  //  функція  рендерить галерею на основі отриманих даних
